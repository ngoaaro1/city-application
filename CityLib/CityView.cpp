/**
 * @file CityView.cpp
 * @author Charles B. Owen & Aaron Ngo
 */
#include "pch.h"
...
{
    mGrabbedItem = mCity.HitTest(event.GetX(), event.GetY());
    AmbulanceLaunch visitor(mGrabbedItem.get());
    mCity.Accept(&visitor);
    TileLandable* destination = visitor.GetDestination();
    TileLandable* launch = visitor.GetAmbulanceLocation();

    auto ambulance = launch->GetAmbulance();
    if (!ambulance->InFlight() && destination != nullptr)
    {
        destination->SetAmbulance(ambulance);
        ambulance->SetLandingTile(destination);
    }
}
...
/**
 * Handle the Buildings>Count menu option
 * @param event Menu event
 */
